...
1.1 输出能力：持续电流20A，短时电流25A（不少于10秒）；
1.2 电源输入：2-3节锂电池组或5—9节镍氢/镍镉电池组；
1.3 BEC输出：5V@2A （线性稳压模式－linear mode）；
1.4 油门信号频率范围（Refresh rate）：50Hz—432Hz；
1.5 最高转速：2极马达210000转/分钟，6极马达70000转/分钟，12极马达35000转/分钟；
1.6 尺寸： 42mm（长）*25mm（宽）*8mm（高）；
1.7 重量： 19g；
*/
#if defined(PROMINI) && defined(ESC_SKYWALKER_432HZ)
volatile uint8_t atomicPWM_PIN11_lowState;
volatile uint8_t atomicPWM_PIN11_highState;
volatile uint8_t atomicPWM_PIN3_lowState;
volatile uint8_t atomicPWM_PIN3_highState;
#endif
...
#if (NUMBER_MOTOR > 2)
#ifndef EXT_MOTOR_RANGE
#if defined(ESC_SKYWALKER_432HZ)
    atomicPWM_PIN11_highState = (motor[2] >> 3);
    atomicPWM_PIN11_lowState  = 255 - atomicPWM_PIN11_highState;
#else
...
#ifndef EXT_MOTOR_RANGE
#if defined(ESC_SKYWALKER_432HZ)
    atomicPWM_PIN3_highState = (motor[3] >> 3);
...
// 400Hz Soft PWM for Pin 11
#if defined(PROMINI) && defined(ESC_SKYWALKER_432HZ) && (NUMBER_MOTOR > 2)
ISR(TIMER2_COMPA_vect)
{
    ...
    if (state == 0)
    {
        ...
        OCR2A += atomicPWM_PIN11_highState;
        state = 1;
    }
    else if (state == 1)
    {
        PORTB &= ~(1 << 3); //Pin 11 Low
        OCR2A += atomicPWM_PIN11_lowState;
        state = 2;
    }
    else if (state == 2)
    {
        //OCR2A += 57; // wait a little to get 400Hz
        OCR2A += 35; // wait a little to get 432Hz
        state = 0;
    }
}
#endif
#if defined(PROMINI) && defined(ESC_SKYWALKER_432HZ) && (NUMBER_MOTOR > 3)
ISR(TIMER2_COMPB_vect)
{
    ...
    if (state == 0)
    {
        ...
        OCR2B += atomicPWM_PIN3_highState;
        state = 1;
    }
    else if (state == 1)
    {
        PORTD &= ~(1 << 3); //Pin 3 Low
        OCR2B += atomicPWM_PIN3_lowState;
        state = 2;
    }
    else if (state == 2)
    {
        //OCR2B += 57; // wait a little to get 400Hz
        OCR2B += 35; // wait a little to get 432Hz
        state = 0;
    }
}
#endif
...
void initOutput()
{
    ...
    TCCR1B |= (1 << WGM13);
    //ICR1 |= 312; // TOP to 312 (for 400Hz);
    ICR1 |= 290; // 289.351851851852, TOP to 290 (for 432Hz);
#endif
...
#if (NUMBER_MOTOR > 3)
#if defined(ESC_SKYWALKER_432HZ)
    TIMSK2 |= (1 << OCIE2B); // Enable CTC interrupt
#else
...
#endif
        if (YAWMOTOR && rcData[THROTTLE] < conf.minthrottle)
        {
            servo[5] =  MINCOMMAND;
        }
        else
...
#else
        // SORRY! I want to stop the motors when stick in low position
        if (
            (rcData[THROTTLE] < MINCHECK)
            /*
            &&
            (
              (!f.BARO_MODE) // in BARO mode, we can stop motor
              || (f.BARO_MODE && (SonarErrors < SONAR_ERROR_MAX && SonarAlt < FAILSAFE_SONAR_ALT)) // in BARO mode, near the ground
            )
            */
        )
#endif
