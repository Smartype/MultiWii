...
void ACC_Common()
{
    ...
    if (InflightcalibratingA > 0)
    {
        ...
        {
            f.ACC_INFLIGHT_CALI_MEASUREMENT_DONE = 1;
#if defined(BUZZER)
    ...
    // Calculate average, shift Z down by acc_1G and store values in EEPROM at end of calibration
    if (f.ACC_INFLIGHT_CALI_SAVE == 1)   //the copter is landed, disarmed and the combo has been done again
    {
...
inline void Optflow_update() {
    ...
    {    
        // send angle info
        uint8_t *varptr = (uint8_t *)&(att.angle[ROLL]);
        i2c_write(*varptr++);
        i2c_write(*varptr);

        varptr = (uint8_t *)&(att.angle[PITCH]);
        i2c_write(*varptr++);
        i2c_write(*varptr);
...
void Optflow_update()
{
    ...
    if (f.ANGLE_MODE == 1 && f.OPTFLOW_MODE == 1 && f.GPS_HOME_MODE == 0)
    {
        ...
        else
        {
            ...
            if (abs(rcCommand[ROLL]) < OF_DEADBAND && abs(rcCommand[PITCH]) < OF_DEADBAND)
            {
                ...
                for (axis = 0; axis < 2; axis++)
                {
                    ...
                    optflow_angle[axis] = EstHVel[axis] * conf.P8[PIDVEL] / 50;  // 16 bit ok: 100 * 200 = 20000
                }
            }
            else
            {
...
inline void optflow_get_vel()
{
    ...
    if (cosZ > 70 && avgSqual.res > 10)
    {
        ...
        alt = constrain((int16_t)EstAlt, 30, (300+150)) * min(avgSqual.res, 50) * 2; // 16 bit ok: 300 * 50 * 2 = 30000;
    }
    else
    {
        alt = 0;
    ...
    for (axis = 0; axis < 2; axis++)
    {
        ...
        if (alt != 0)
        {
            ...
            vel_of[axis] = ((int32_t)optflow_pos[axis] + (att.angle[axis] - prevAngle[axis]) * 17 ) * alt / dTime ;
            /*
                              X  Y
...
void Optflow_init()
{
    ...
    if (ADNS_read(Product_ID) == 0x12)
    {
        ...
        1=1000cpi
        */
        ADNS_write(Mouse_Control, RES_CFG); // Set resolution
    }
}
